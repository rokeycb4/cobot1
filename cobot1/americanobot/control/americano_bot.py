...
def main(args=None):
    ...
    # 기존 main 내부 로직은 그대로 유지됨
    tp_log("main called")
    movej(posj(0,0,90, 0,90,0)) # 홈 이동
    release()


    # 외력 인식해서 제조 수행: 왼쪽에서 밀기
    force_control_on(0)
    while rclpy.ok():
        if check_force_condition(DR_AXIS_Z, min=10, ref=DR_TOOL):
            tp_log("커피 제조 로봇 종료")
            force_control_off()
            break

        elif check_force_condition(DR_AXIS_X, min=5, ref=DR_TOOL):
            force_control_off()
            make_americano()

        elif check_force_condition(DR_AXIS_X, min=5, ref=DR_TOOL):  # 반대방향도 같은 min값으로 사용
            force_control_off()
            make_latte()
